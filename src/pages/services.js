    ...
    <main className={styles.page}>
      ...
      <section className={styles.catalog}>
        ...
        <div className={styles.grid}>
          {visibleServices.map((service) => <article key={service.name} className={styles.card}><span className={styles.marker} style={{backgroundImage: `url(${baseUrl}${(service.image || serviceIcons[service.name] || '').replace(/^\//, '')})`}} aria-hidden="true" /><h2>{service.name}</h2><p>{service.description}</p>{service.url ? <a className={styles.learn} href={service.url} target="_blank" rel="noreferrer">Explore service &#8594;</a> : <span className={styles.learn}>Learn More &#8594;</span>}</article>)}
        </div>
      </section>
    </main>
  </Layout>;
